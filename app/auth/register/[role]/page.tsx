...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  const renderFormContent = () => {
    ...
    // ... (Kode LPK dan Perusahaan biarkan sama seperti sebelumnya agar tidak rusak)
    else if (role === 'lpk') {
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><label className="text-xs font-bold block mb-1">Nama Lembaga (LPK)</label><input required name="name" onChange={handleChange} className="w-full border rounded p-2 text-sm"/></div>
          <div><label className="text-xs font-bold block mb-1">No. VIN / Izin</label><input required name="vin" onChange={handleChange} className="w-full border rounded p-2 text-sm"/></div>
          <div className="md:col-span-2"><label className="text-xs font-bold block mb-1">No. Telepon Admin</label><input required name="phone" onChange={handleChange} className="w-full border rounded p-2 text-sm"/></div>
        </div>
      )
    } else {
      return (
