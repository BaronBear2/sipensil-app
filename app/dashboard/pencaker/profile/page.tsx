...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      if (profile) {
        setFormData(profile)
        // Cek apakah user sedang dalam mode "Pending" (Sedang diverifikasi)
        // Jika Pending, user TIDAK BOLEH edit data.
  ...
  // 2. LOGIC FORM (Sama persis dengan script asli)
  ...
  const executeSave = async () => {
    ...
    // Logic: Setiap kali simpan, status kembali jadi PENDING agar Admin cek ulang
    ...
    if (error) {
      ...
      setFormData(prev => ({ ...prev, account_status: 'pending' }))
      setIsEditing(false)
  ...
  // Logic Styling (Agar sama dengan script asli)
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100 mb-6">
           ...
           <div className={`px-6 py-4 text-sm font-bold flex flex-col md:flex-row justify-between items-center border-b ${getStatusColor(formData.account_status || 'unverified')}`}>
              ...
              {/* Tombol Edit Logic */}
              {!isEditing && formData.account_status !== 'pending' && (
              ...
              {!isEditing && formData.account_status === 'pending' && (
                 <span className="flex items-center gap-1 text-xs opacity-80"><Lock size={12}/> Data dikunci (Sedang Verifikasi)</span>
              )}
              {isEditing && (
