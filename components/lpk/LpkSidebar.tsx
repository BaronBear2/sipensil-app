...
import { Home, User, FileText, LayoutDashboard, LogOut, ChevronDown, ChevronRight, Menu, X, ClipboardList } from 'lucide-react'
...
const MENU_ITEMS: MenuItem[] = [
    ...
    { name: 'Dashboard', href: '/dashboard/lpk', icon: LayoutDashboard },
    { name: 'Profil Saya', href: '/dashboard/lpk/profile', icon: User },
    {
        name: 'Laporan Periodik',
        icon: FileText,
        children: [
            { name: 'Buat Laporan', href: '/dashboard/lpk/laporan/create' },
            { name: 'Riwayat Laporan', href: '/dashboard/lpk/laporan/riwayat' },
        ]
    }
]
        ...
        <>
            ...
            `}>
                ...
                <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
                    <div className="mb-2 px-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Menu LPK</div>
            ...
            <div className="space-y-1">
                <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-2.5 rounded-xl text-sm font-bold transition-all 
                        ${isActive(item) ? 'bg-emerald-50 text-emerald-800' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}
                        ${depth > 0 ? 'text-xs my-0.5' : ''}
                    `}
                    style={{ paddingLeft: `${16 + (depth * 12)}px` }}
                >
                    <div className="flex items-center gap-3">
                        {item.icon && <item.icon size={18} className={isActive(item) ? 'text-emerald-600' : 'text-gray-400'} />}
                        <span>{item.name}</span>
                    </div>
    ...
    return (
        <Link
            ...
            className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-bold transition-all
                ${isCurrentActive ? 'bg-emerald-50 text-emerald-600 shadow-sm' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'}
                ${depth > 0 ? 'text-xs my-0.5' : ''}
            `}
            style={{ paddingLeft: `${16 + (depth * 12)}px` }}
        >
            {item.icon && <item.icon size={18} className={isCurrentActive ? 'text-emerald-600' : 'text-gray-400'} />}
            {!item.icon && depth > 0 && <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${isCurrentActive ? 'bg-emerald-400' : 'bg-gray-300'}`}></span>}
            {item.name}
        </Link>
    )
}
