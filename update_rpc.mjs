...
BEGIN
    ...
    WHERE tr.training_id = bt.id
      ...
      AND (
          (ts.last_selection_date IS NOT NULL AND CURRENT_DATE > ts.last_selection_date)
          OR
          (ts.last_selection_date IS NULL AND bt.training_start_date IS NOT NULL AND CURRENT_DATE >= bt.training_start_date)
      );
    ...
    WHERE tr.training_id = bt.id
      ...
      AND tr.status = 'DITERIMA'
      AND (
          (te.exam_date IS NOT NULL AND CURRENT_DATE >= te.exam_date)
          OR
          (te.exam_date IS NULL AND bt.training_end_date IS NOT NULL AND CURRENT_DATE >= (bt.training_end_date - INTERVAL '2 days'))
      );
    ...
    WHERE tr.training_id = bt.id
      ...
      AND (
          (te.exam_date IS NOT NULL AND CURRENT_DATE > te.exam_date)
          OR
          (te.exam_date IS NULL AND bt.training_end_date IS NOT NULL AND CURRENT_DATE > bt.training_end_date)
      );
END;
$$ LANGUAGE plpgsql;
`
...
async function main() {
    // Attempt to execute via RPC. If this fails, we will just rely on the fact that I modified the migration file
    // and the user can run it. Wait, the user has a local instance or remote? 
    // Usually Supabase has a migration CLI.
}
main()
